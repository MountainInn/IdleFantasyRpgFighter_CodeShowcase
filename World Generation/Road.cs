...
public class Road : MonoBehaviour
{
    ...
    public List<SplineData<float>> Widths
    {
        ...
        public Spline singleSpline => LoftSplines.Single();
        ...
        public void OnEnable()
        {
            ...
            meshRenderer.transform.position = meshRenderer.transform.position.WithY(0);
        }
        ...
        public void AppendKnot(Vector3 position)
        {
            var spline = LoftSplines.First();

            int count = spline.Count;

            spline.Insert(count -1, new BezierKnot(position));
        }
        ...
        public void LoftAllRoads()
        {
            float totalLength =
                LoftSplines
                .Aggregate(0f, (length, spline) => length + spline.GetLength());

            m_TextureScale = totalLength / 10f;
            ...
            LoftMesh.RecalculateNormals();

            meshFilter.sharedMesh = m_Mesh;


            meshRenderer.transform.position = meshRenderer.transform.position.WithY(-transform.position.y);
        }
        ...
        public void Loft(Spline spline, int widthDataIndex)
        {
            ...
            Vector3 originalPosition = treadmill.GetAbsolutePosition(transform);
            ...
            float3 leftPos = roadCaster.CastDown(originalPosition,
                                                 pos - (tangent * w),
                                                 verticalPadding );

            float3 rightPos = roadCaster.CastDown(originalPosition,
                                                  pos + (tangent * w),
                                                  verticalPadding);

            m_Positions.Add(leftPos);
            m_Positions.Add(rightPos);
            m_Normals.Add(up);
            m_Normals.Add(up);
        ...
        public class Pool : PoolOnTreadmill<Road>
        {
        }
}
